...
CACHE_DIR = Path("data/processed/live_cache")
SQLITE_DB_PATH = CACHE_DIR / "live_data_cache.sqlite"
PARQUET_SEC_PATH = "data/processed/sec_dataset/v2_sec_financials_pivoted_clean.parquet"
...
# Defaults canonicos de las 21 variables optimas segun preprocessor_pipeline.joblib
...
def get_macro_gmm_detector():
    ...
    if _macro_gmm_detector is None:
        try:
            ...
            import joblib
            for p in ["models/macro_gmm_regimes_model.pkl", "Entregable/models/macro_gmm_regimes_model.pkl"]:
                if os.path.exists(p):
                    _macro_gmm_detector = joblib.load(p)
...
def get_corp_gmm_detector():
    ...
    if _corp_gmm_detector is None:
        try:
            ...
            import joblib
            for p in ["models/corp_archetypes_gmm_model.pkl", "Entregable/models/corp_archetypes_gmm_model.pkl"]:
                if os.path.exists(p):
                    _corp_gmm_detector = joblib.load(p)
...
def _try_duckdb_accounting(clean_ticker: str, clean_name: str, defaults: dict) -> Optional[Tuple[str, int, dict]]:
    if not os.path.exists(PARQUET_SEC_PATH):
        return None
    try:
        import duckdb
        con = duckdb.connect()
